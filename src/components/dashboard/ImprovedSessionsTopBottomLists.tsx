...
export const ImprovedSessionsTopBottomLists: React.FC<ImprovedSessionsTopBottomListsProps> = ({
  ...
  const processedData = useMemo(() => {
    ...
    data.forEach(session => {
      ...
      // Check if this session already exists in the group (avoid duplicates)
      const existingSession = grouped[key].uniqueSessions.find(s => 
        s.date === session.date && s.sessionId === session.sessionId
      );
      
      if (!existingSession) {
        grouped[key].uniqueSessions.push(session);
    ...
    // Calculate averages using unique sessions
    Object.values(grouped).forEach(item => {
      const totalCapacity = item.uniqueSessions.reduce((sum, s) => sum + s.capacity, 0);
    ...
    // Filter out rows with less than 2 sessions
    const filteredData = Object.values(grouped).filter(item => {
      return item.sessions >= 2;
    });
  ...
  return (
    <div data-export-container data-container-name={`${title} - ${variant} performers`}>
      <Card className="bg-white shadow-sm border border-gray-200">
        ...
        <CardContent className="p-0" data-export-table data-table-name={`${title} List`}>
