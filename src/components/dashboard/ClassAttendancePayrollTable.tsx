...
export const ClassAttendancePayrollTable: React.FC<ClassAttendancePayrollTableProps> = ({ data, location }) => {
  ...
  const trendsData = useMemo(() => {
    ...
    processedData.forEach(item => {
      ...
      const prevData = processedData.find(p => 
      ...
      if (prevData) {
        const key = `${item.location}-${item.monthYear}`;
        trends[key] = {
          sessionsChange: prevData.totalSessions > 0 ? 
            Number((((item.totalSessions - prevData.totalSessions) / prevData.totalSessions) * 100).toFixed(1)) : 0,
          avgChange: prevData.classAvgWithEmpty > 0 ? 
            Number((((item.classAvgWithEmpty - prevData.classAvgWithEmpty) / prevData.classAvgWithEmpty) * 100).toFixed(1)) : 0,
          revenueChange: prevData.totalPaid > 0 ? 
            Number((((item.totalPaid - prevData.totalPaid) / prevData.totalPaid) * 100).toFixed(1)) : 0,
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50/80 hover:bg-slate-50">
                  <TableHead 
                    className="cursor-pointer text-xs font-semibold text-slate-700 h-10 px-4"
                  ...
                  <TableHead 
                    className="cursor-pointer text-xs font-semibold text-slate-700 h-10 px-4 text-center"
                  ...
                  <TableHead 
                    className="cursor-pointer text-xs font-semibold text-slate-700 h-10 px-4 text-center"
                    onClick={() => handleSort('emptySession')}
                  >
                    Late Cancelled
                  </TableHead>
                  <TableHead 
                  ...
                  <TableHead 
                    className="cursor-pointer text-xs font-semibold text-slate-700 h-10 px-4 text-center"
                    onClick={() => handleSort('classAvgWithEmpty')}
                  >
                    <Tooltip>
                  ...
                  <TableHead 
                    className="cursor-pointer text-xs font-semibold text-slate-700 h-10 px-4 text-center"
                    onClick={() => handleSort('classAvgWithoutEmpty')}
                  >
                    <Tooltip>
                      <TooltipTrigger className="flex items-center gap-1">
                        Avg w/o Empty
                  ...
                  <TableHead className="text-xs font-semibold text-slate-700 h-10 px-4 text-center">
                    Trends
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {processedData.map((row, index) => {
                  ...
                  return (
                    <TableRow 
                      key={index} 
                      className="table-row-fixed hover:bg-blue-50/50 transition-colors border-b border-slate-100"
                    >
                      <TableCell className="table-cell-compact text-xs font-medium text-slate-800">
                        <div className="table-cell-nowrap max-w-[120px]">
                          {row.location}
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="table-cell-compact text-xs text-slate-600">
                        <div className="table-cell-nowrap max-w-[100px]">
                          {row.teacherName}
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="table-cell-compact text-xs text-center">
                        <Badge 
                          variant={row.emptySession > 5 ? "destructive" : "secondary"}
                          className="text-xs"
                        >
                          {row.emptySession}
                        </Badge>
                      ...
                      <TableCell className="table-cell-compact text-xs text-center">
                        <div className="flex flex-col items-center gap-1">
                          <Badge 
                            variant={row.fillRate >= 80 ? "default" : row.fillRate >= 60 ? "secondary" : "destructive"}
                            className="text-xs"
                          >
                            {row.fillRate}%
                          </Badge>
                      ...
                      <TableCell className="table-cell-compact text-xs text-center">
                        <Badge 
                          variant={row.cancelRate <= 10 ? "default" : row.cancelRate <= 20 ? "secondary" : "destructive"}
                          className="text-xs"
                        >
                          {row.cancelRate}%
                        </Badge>
